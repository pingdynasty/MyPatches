...
public:
    ...
    std::vector<EndpointDetails> getInputEndpoints() const
    {
        return
        {
            ...
            { "cutoffFrequency", "in:cutoffFrequency", EndpointType::event,  "float32", 0, "{ \"name\": \"Cutoff\", \"min\": 20, \"max\": 20000, \"init\": 10000, \"step\": 10 }" },
            { "gaindB",          "in:gaindB",          EndpointType::event,  "float32", 0, "{ \"name\": \"Gain\", \"min\": 0, \"max\": 40, \"init\": 20, \"step\": 0.1 }"         }
        };
    }
    ...
    {
        {  "cutoffFrequency",  "Cutoff",  "",  20.0f,  20000.0f,  10.0f,  10000.0f,  true,  false,  false,  "",  ""  },
        {  "gaindB",           "Gain",    "",  0.0f,   40.0f,     0.1f,   20.0f,     true,  false,  false,  "",  ""  }
    };
    ...
    std::vector<Parameter> createParameterList()
    {
        return
        {
            {  parameters[0],  10000.0f,  [this] (float v) { addInputEvent_cutoffFrequency (v); }  },
            {  parameters[1],  20.0f,     [this] (float v) { addInputEvent_gaindB (v); }           }
        };
    }
